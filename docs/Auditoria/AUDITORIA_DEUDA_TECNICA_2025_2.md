...
Se identificaron **39 hallazgos** clasificados en 4 niveles de severidad:

| Severidad | Cantidad | Descripción |
|-----------|----------|-------------|
| CRÍTICO   | 7        | Funcionalidad definida pero NO implementada, o lógica de negocio rota |
...
### C-07: Chat — Funcional en apariencia, inutilizable en la práctica

**Archivos:** `Services/ChatService.cs`, `Components/Pages/MisObjetivos/ObjetivoDetalle.razor`

**Problema:** El chat tiene código de envío y lectura, pero tiene 5 fallas que lo hacen no funcional como herramienta de comunicación real:

1. **Sin actualización en tiempo real:** No hay SignalR, polling ni timer. La conversación solo se refresca cuando el usuario envía un mensaje propio (`await LoadData()` post-envío). Si el jefe escribe, el empleado no ve nada hasta que él mismo envíe algo. Es un chat "ciego".

2. **Acoplado a ObjetivoDetalle:** Solo se puede chatear desde la vista de detalle de un objetivo específico (`/seguimientos/{EmpleadoId}/objetivo/{ObjetivoId}`). No existe bandeja de conversaciones, ni acceso al chat desde el menú principal, ni desde la lista de seguimientos. Si tienes 3 objetivos con un empleado, hay 3 "chats" que en realidad son la misma conversación (filtrada por JefeId+EmpleadoId, no por ObjetivoId).

3. **No marca mensajes como leídos:** El campo `MensajeChat.Leido` existe en el modelo y tiene datos seed con `Leido=false`, pero el código nunca ejecuta un UPDATE para marcar mensajes como leídos cuando se visualiza la conversación.

4. **Sin indicador de mensajes pendientes:** En la lista de empleados de Seguimientos no se muestra badge ni indicador de mensajes sin leer. El jefe no sabe si un empleado le escribió sin entrar a cada objetivo uno por uno.

5. **Sin auto-scroll:** El contenedor del chat tiene `overflow-y: auto` pero no hay JavaScript que haga scroll al fondo al cargar la página ni al enviar un mensaje nuevo. Los mensajes recientes quedan fuera de vista.

**Severidad:** CRÍTICO — el chat existe en la UI pero no cumple su propósito funcional de comunicación bidireccional en tiempo real.
...
### ETAPA 10 — CHAT FUNCIONAL

**Prompt para Claude Code:**
```
El chat actual está acoplado a ObjetivoDetalle y no funciona como herramienta 
de comunicación real. Refactorízalo:

1. Ampliar ChatService.cs:
   - MarcarMensajesComoLeidosAsync(int jefeId, int empleadoId, bool esJefe)
     → marcar como leídos los mensajes donde el usuario es destinatario
   - GetConversacionesConNoLeidosAsync(int jefeId)
     → retornar lista de empleados con count de mensajes no leídos
   - GetMensajesNoLeidosCountAsync(int usuarioId, bool esJefe)
     → para badge global en header

2. En ObjetivoDetalle.razor (chat existente):
   - Al cargar la conversación, llamar a MarcarMensajesComoLeidosAsync
   - Agregar timer de auto-refresh cada 10 segundos:
     private Timer? _chatTimer;
     OnInitialized → _chatTimer = new Timer(async _ => { 
       mensajes = await ChatService.GetConversacionAsync(...);
       await InvokeAsync(StateHasChanged); 
     }, null, 10000, 10000);
     Implementar IDisposable para limpiar el timer
   - Agregar auto-scroll al fondo del chat:
     Después de LoadData y OnSendChat, invocar JS:
     await JS.InvokeVoidAsync("scrollToBottom", "chat-container");
     Crear la función JS en wwwroot (o en _Host.cshtml):
     window.scrollToBottom = (id) => { 
       var el = document.getElementById(id); 
       if(el) el.scrollTop = el.scrollHeight; 
     };

3. En Seguimientos/EmpleadoDetalle.razor:
   - Agregar badge de mensajes no leídos junto al nombre del empleado
     en la lista de seguimientos

4. En Seguimientos/Index.razor:
   - Mostrar indicador de mensajes pendientes por empleado
   - Usar GetConversacionesConNoLeidosAsync para obtener los counts

5. (Opcional avanzado) Crear Components/Shared/ChatPanel.razor:
   - Componente reutilizable de chat que reciba jefeId y empleadoId
   - Extraer la lógica del chat de ObjetivoDetalle para reutilizar
   - Considerar agregar como panel lateral accesible desde cualquier página
```

---

### ETAPA 11 — CONSISTENCIA Y LIMPIEZA
...
Etapa 0 (Decisión humana)
   ...
   │
   └──→ Etapa 11 (Limpieza) — ejecutar al final
```
