...
public static class SeedData
{
    public static async Task InitializeAsync(AppDbContext db, IWebHostEnvironment env, IConfiguration config)
    {
        ...
        var configsRequeridas = new List<ConfiguracionPlataforma>
        {
            ...
            new ConfiguracionPlataforma { Clave = "jefe_puede_crear_objetivos", Valor = "true", Descripcion = "Permite a jefes crear objetivos para sus usuarios", Tipo = "boolean" },
            new ConfiguracionPlataforma { Clave = "empleado_puede_autoevaluar", Valor = "true", Descripcion = "Habilita el módulo de autoevaluación para usuarios", Tipo = "boolean" },
            ...
            new ConfiguracionPlataforma { Clave = "un_objetivo_por_pilar", Valor = "false", Descripcion = "Si es true, un usuario solo puede tener un objetivo activo por pilar por año", Tipo = "boolean" },
        ...
        // Si NO es BD nueva, no procesar jefes/usuarios (ya existen)
        if (!esBdNueva) return;

        // ─── Classify people: Jefes vs Usuarios ───
        // Roles that imply management: "Usuario", "Gerente", "Director", "Director General"
        var jefeRoles = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            { "Usuario", "Gerente", "Director", "Director General", "RRHH" };

        // First pass: create Jefes
        var jefeMap = new Dictionary<string, Usuario>(StringComparer.OrdinalIgnoreCase);
        int jefeIdCounter = 1;
        ...
        db.Usuarios.AddRange(jefeMap.Values);
        await db.SaveChangesAsync();
        ...
        // Second pass: create Usuarios (Colaboradores)
        int empIdCounter = jefeIdCounter;
        ...
        foreach (var row in rows)
        {
            ...
            // Skip if email already used by a Usuario
            if (jefeMap.ContainsKey(row.Mail.ToLower())) continue;
